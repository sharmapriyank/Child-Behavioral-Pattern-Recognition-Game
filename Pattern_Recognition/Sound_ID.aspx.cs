...
public partial class Sound_ID : System.Web.UI.Page
{
    ...
    private void LoadQuestion()
    {
        try
        {
            ...
            {
                sql = string.Format("select GE,Correct_Answer,Sound_FileName, UID from Sound_ID where GE>={0} and uid NOT IN (Select QuestionId From Student_ViewLog Where Child_Id={1} AND QuestionType={2}) ORDER BY GE", ge, userInfo.ChildId, questionType);
            }
            else
            {
                sql = string.Format("select GE,Correct_Answer,Sound_FileName, UID from Sound_ID where GE<={0} and uid NOT IN (Select QuestionId From Student_ViewLog Where Child_Id={1} AND QuestionType={2}) ORDER BY GE DESC", ge, userInfo.ChildId, questionType);
            }
