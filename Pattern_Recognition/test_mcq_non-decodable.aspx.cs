...
public partial class test_mcq_non_decodable : System.Web.UI.Page
{
    Conn con = new Conn();
    ...
    private void LoadQuestion()
    {
        try
        {
            ...
            {
                sql = string.Format("select UID, GE, option1, option2, option3, option4, option5, option6, option7, Correct_Answer, audioFileName FROM mcq_nondecodable_words_audio where GE>={0} and uid NOT IN (Select QuestionId From Student_ViewLog Where Child_Id={1} AND QuestionType={2}) ORDER BY GE", ge, userInfo.ChildId, questionType);
            }
            else
            {
                sql = string.Format("select UID, GE, option1, option2, option3, option4, option5, option6, option7, Correct_Answer, audioFileName FROM mcq_nondecodable_words_audio where GE<={0} and uid NOT IN (Select QuestionId From Student_ViewLog Where Child_Id={1} AND QuestionType={2}) ORDER BY GE DESC", ge, userInfo.ChildId, questionType);
            }
